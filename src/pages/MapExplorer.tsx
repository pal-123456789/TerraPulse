...
const MapExplorer = () => {
  ...
  if (loading) {
    return (
      <PageLoader label="Loading Global Map" />
    );
  }
  ...
  return (
    <div className="min-h-screen bg-space-gradient relative overflow-hidden">
      ...
      <SEO
        title={"Live Anomaly Map — Real-Time Earth Monitoring"}
        description={"Interactive global map of live environmental anomalies — wildfires, storms, floods — powered by NASA data. Terra Guardians by Pal Ghevariya."}
        path={"/map"}
        image={"https://www.terraguardians.us/og-map.jpg"}
        keywords={"live map, environmental anomalies, wildfire map, storm tracker, Pal Ghevariya, Terra Guardians, TerraPulse"}
        schema={{
          "@context": "https://schema.org",
          "@type": "WebPage",
          name: "Live Anomaly Map",
          url: "https://www.terraguardians.us/map",
          isPartOf: { "@id": "https://www.terraguardians.us/#website" },
          about: "Real-time environmental anomaly map with NASA and OpenWeather data",
          primaryImageOfPage: "https://www.terraguardians.us/og-map.jpg",
        }}
      />
      <Navigation />
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        const pool = filteredAnomalies.length ? filteredAnomalies : anomalies;
                        if (!pool.length) {
                          toast.info("No anomalies to jump to");
                          return;
                        }
                        const pick = pool[Math.floor(Math.random() * pool.length)];
                        setSelectedAnomaly(pick);
                        toast.success(`Jumped to ${pick.name}`);
                      }}
                      title="Jump to a random anomaly"
                    >
                  ...
                  {filteredAnomalies.map((anomaly, i) => {
                    ...
                    return (
                      <motion.button
                        ...
                        aria-label={`Select anomaly ${anomaly.name}, ${anomaly.severity} severity, ${anomaly.anomaly_type}`}
                        className={`w-full text-left p-3 rounded-lg border cursor-pointer transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background ${
                          isSelected
