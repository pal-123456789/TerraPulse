...
const History = () => {
  ...
  // Real DB cleanup of old detected anomalies + predictions
  const cleanupOldRecords = async () => {
    if (!confirm(`Delete ALL detected anomalies and predictions older than ${CLEANUP_THRESHOLD_DAYS} days? This cannot be undone.`)) {
      return;
    }
    ...
    try {
      ...
      // Delete old anomalies (any status, not just resolved)
      const { error: anomErr, count: anomCount } = await supabase
        .from("anomalies")
        .delete({ count: "exact" })
        .lt("detected_at", thresholdIso);
      ...
      // predictions delete may fail silently (RLS), so don't throw

      // Clear local state immediately of non-historical, old entries
      setAnomalies(prev => prev.filter(a =>
        a.id.startsWith("hist-") || new Date(a.detected_at) > thresholdDate
      ));

      const total = (anomCount || 0) + (predCount || 0);
      if (total > 0) {
        toast.success(`Cleaned up ${total} old record${total === 1 ? "" : "s"}`);
      } else {
        toast.info(`No records older than ${CLEANUP_THRESHOLD_DAYS} days to clean`);
      }
  ...
  // Delete a single anomaly (skips static historical entries)
  const deleteAnomaly = async (id: string, name: string) => {
    if (id.startsWith("hist-")) {
      toast.info("Historical records cannot be deleted");
      return;
    }
                                  ...
                                  <div className="flex items-center gap-1 shrink-0">
                                    {!anomaly.id.startsWith("hist-") && (
                                      <Button
                                        ...
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          deleteAnomaly(anomaly.id, anomaly.name);
                                        }}
                                        aria-label={`Delete ${anomaly.name}`}
                                      >
