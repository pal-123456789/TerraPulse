...
const CourseCard = memo(({ course, progress, completedLessons, onStart, onCertificate }: {
  course: Course;
  progress: number;
      ...
      <Card className="glass-ultra hover:border-primary/30 transition-all group cursor-pointer h-full">
        <CardContent className="p-6">
          ...
          {progress >= 100 && (
            <Button
              variant="outline"
              className="w-full mt-2 gap-2 border-primary/40 text-primary hover:bg-primary/10"
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto glass-ultra border-primary/30">
        ...
        <div className="space-y-6">
          ...
          {lesson.content_type === 'video' && (() => {
            // Fallback to curated NASA / climate education videos when DB url is missing
            const fallbackByKeyword: Record<string, string> = {
              climate: 'https://www.youtube.com/embed/G4H1N_yXBiA',
              weather: 'https://www.youtube.com/embed/DweUm0VxYAo',
              ocean: 'https://www.youtube.com/embed/kp7sUdM-Y_0',
              atmosphere: 'https://www.youtube.com/embed/wO61D9x6lNY',
              satellite: 'https://www.youtube.com/embed/qcjIFOsKSV4',
              earth: 'https://www.youtube.com/embed/HCDVN7DCzYE',
            };
            const dbUrl = lesson.content.video_url;
            const isPlaceholder = !dbUrl || dbUrl === 'https://example.com/video';
            const titleLower = lesson.title.toLowerCase();
            const fallbackUrl = Object.entries(fallbackByKeyword).find(([k]) => titleLower.includes(k))?.[1]
              || 'https://www.youtube.com/embed/HCDVN7DCzYE';
            const videoUrl = isPlaceholder ? fallbackUrl : dbUrl;
            return (
            <div className="space-y-4">
...
const Learn = () => {
  ...
  const [userName, setUserName] = useState<string>("TerraGuardian");
  ...
  const fetchUserProgress = useCallback(async () => {
    try {
      ...
      // Try to grab a friendly display name for the certificate
      const meta = (user.user_metadata || {}) as Record<string, string | undefined>;
      const friendly =
        meta.full_name || meta.name || meta.username || user.email?.split("@")[0] || "TerraGuardian";
  ...
  const handleCompleteLesson = useCallback(async (lessonNumber: number) => {
    ...
    const totalLessons = lessonsByCoure[courseId]?.length || 1;

    // Read latest state via functional updater to avoid stale closures
    let newCompletedLessons: number[] = [];
    let newProgress = 0;
    let alreadyDone = false;

    setUserProgress(prev => {
      const existing = prev[courseId] || { course_id: courseId, progress: 0, completed_lessons: [] };
      if (existing.completed_lessons.includes(lessonNumber)) {
        alreadyDone = true;
        newCompletedLessons = existing.completed_lessons;
        newProgress = existing.progress;
        return prev;
      }
      newCompletedLessons = [...existing.completed_lessons, lessonNumber];
      newProgress = Math.min(100, Math.round((newCompletedLessons.length / totalLessons) * 100));
      return {
        ...prev,
        [courseId]: {
          course_id: courseId,
    ...
    try {
      ...
      // Minimal payload — `last_activity` has a default and isn't part of the
      // unique key, so excluding it avoids type coercion issues that caused
      // "Failed to save".
  ...
  return (
    <div className="min-h-screen bg-space-gradient relative overflow-x-hidden">
      ...
      <SEO
        title={"Learn — Environmental Science Courses & Certifications"}
        description={"Free interactive courses on climate science, satellite data, and environmental monitoring. Earn certificates from Terra Guardians, by Pal Ghevariya."}
        path={"/learn"}
        image={"https://www.terraguardians.us/og-learn.jpg"}
        keywords={"environmental courses, climate science learning, certificates, Pal Ghevariya, Terra Guardians, TerraPulse"}
        schema={{
          "@context": "https://schema.org",
          "@type": "CollectionPage",
          name: "Learn with Terra Guardians",
          url: "https://www.terraguardians.us/learn",
          isPartOf: { "@id": "https://www.terraguardians.us/#website" },
          about: "Free courses on climate science, satellite data, and environmental AI",
          primaryImageOfPage: "https://www.terraguardians.us/og-learn.jpg",
        }}
      />
      <Navigation />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {courses.map((course) => (
                <CourseCard
                  ...
                  onCertificate={() => setCertificateCourse(course)}
                />
              ))}
            </div>
          </MotionSection>
      ...
      <Certificate
        ...
        onOpenChange={(o) => !o && setCertificateCourse(null)}
