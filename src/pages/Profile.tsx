          ...
          <div className="max-w-4xl mx-auto animate-fade-in">
            ...
            <Tabs defaultValue="overview" className="space-y-6">
              ...
              <TabsContent value="overview">
                {/* Profile Strength */}
                {(() => {
                  const fields = [profile.username, profile.full_name, profile.bio, profile.location, profile.avatar_url];
                  const filled = fields.filter(Boolean).length;
                  const strength = Math.round((filled / fields.length) * 100);
                  const level = Math.max(1, Math.floor(
                    (userStats.coursesCompleted * 3) +
                    (userStats.reportsSubmitted * 2) +
                    (userStats.commentsPosted * 1) +
                    Math.floor(userStats.totalProgress / 20)
                  ));
                  return (
                    <Card className="glass-panel border-primary/20 mb-6 p-5">
                      <div className="flex flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-primary/30 to-accent/20 flex items-center justify-center border border-primary/40">
                            <Award className="w-6 h-6 text-primary" />
                          </div>
                          <div>
                            <p className="text-sm text-muted-foreground">Guardian Level</p>
                        ...
                        <div className="flex-1 min-w-[200px]">
                          <div className="flex justify-between text-xs mb-1.5">
                            ...
                            <span className="text-foreground font-semibold">{strength}%</span>
                          </div>
                          <Progress value={strength} className="h-2" />
                          {strength < 100 && (
                            <p className="text-[11px] text-muted-foreground mt-1.5">
                              Complete your profile to unlock all Guardian perks.
                            </p>
                          )}
                        </div>
                      </div>
