...
export const AuthModal = ({ open, onOpenChange }: AuthModalProps) => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Reset link sent. Check your email and click the link to set a new password.");
      setShowForgot(false);
    } catch (err: any) {
  ...
  const handleSignUp = async (e?: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      ...
      // Otherwise the signup email carries a 6-digit verification code.
      setSignupStep("otp");
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // "signup" is the token type Supabase issues for the confirmation email;
      // fall back to "email" (magic-link/OTP) if the account is already confirmed.
