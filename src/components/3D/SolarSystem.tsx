      ...
      <group ref={orbitRef}>
        <group rotation={[0, 0, planet.tilt ?? 0]}>
          <mesh ref={meshRef} castShadow receiveShadow>
            <sphereGeometry args={[planet.size, isMobile ? 64 : 96, isMobile ? 64 : 96]} />
...
const Scene = () => (
  <>
    ...
    <Stars radius={180} depth={80} count={isMobile ? 900 : 2800} factor={4.5} fade speed={0.35} />
    <Sun />
    {PLANETS.map((p, i) => (
...
const SolarSystem = ({ height = "min(780px, 82vh)", className = "" }: SolarSystemProps) => {
  return (
    <div
    ...
    >
      <Canvas
        shadows={!isMobile}
        camera={{ position: [0, 8.5, 19], fov: 52 }}
      ...
      >
        <Suspense fallback={<LoadingFallback />}>
          <Scene />
          <OrbitControls enablePan={false} enableZoom minDistance={7} maxDistance={34} autoRotate autoRotateSpeed={0.22} enableDamping dampingFactor={0.06} />
        </Suspense>
      </Canvas>
