...
import { Plus, Download, Bell, Share2, Map, BarChart3, Sparkles, Radio } from "lucide-react";
...
export const QuickActions = () => {
  ...
  // Lift the FAB above any bottom-right sonner toasts so the + is never covered.
  // Auto-returns to the resting position when toasts disappear.
  useEffect(() => {
    let raf = 0;
    ...
    const measure = () => {
      const toaster = document.querySelector<HTMLElement>(
        'ol[data-sonner-toaster][data-y-position="bottom"][data-x-position="right"]'
      );
      ...
      const toasts = toaster.querySelectorAll<HTMLElement>('li[data-sonner-toast]');
      if (toasts.length === 0) {
        setToastOffset(0);
        return;
      }
      const rect = toaster.getBoundingClientRect();
      setToastOffset(Math.max(0, Math.round(rect.height) + 12));
    };
  ...
  const actions = [
    ...
    },
    {
      icon: Radio,
      label: "Response Hub",
      color: "bg-fuchsia-500 text-white hover:bg-fuchsia-500/90",
      action: () => { setIsOpen(false); navigate("/response-hub"); },
    },
    {
      icon: BarChart3,
      label: "View Stats",
  ...
  return (
    <>
      ...
      <div
        className="fixed bottom-6 right-6 md:bottom-8 md:right-8 z-50 transition-transform duration-300 ease-out"
        style={{ transform: `translateY(-${toastOffset}px)` }}
      >
        <div
          className={cn(
        ...
        <div className="relative">
          ...
          >
            ...
            {/* Shimmer sweep */}
            <span className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform [transition-duration:1100ms] bg-gradient-to-r from-transparent via-white/25 to-transparent" />
