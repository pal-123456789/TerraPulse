...
export const Globe3D = ({ className = '' }: { className?: string }) => {
  ...
  // Pre-flight WebGL support probe — avoid mounting <Canvas> at all when the
  // browser/device cannot create a context (older iOS, hardened browsers, VMs).
  useEffect(() => {
    try {
  ...
  if (webglFailed) {
    return (
      <div
        ...
        role="img"
        aria-label="Terra Guardians globe (3D preview unavailable on this device)"
      >
        <div
          aria-hidden
          ...
          style={{
            background:
              'radial-gradient(circle at 50% 50%, hsla(180,100%,50%,0.28) 0%, hsla(270,70%,40%,0.14) 40%, transparent 70%)',
            filter: 'blur(24px)',
          }}
        />
        <div className="relative z-10 text-center px-6">
          <div className="mx-auto mb-4 w-40 h-40 rounded-full bg-[radial-gradient(circle_at_30%_30%,#00d4ff,transparent_60%),radial-gradient(circle_at_70%_70%,#aa00ff,transparent_65%),#061225] border border-primary/30 shadow-[0_0_60px_hsla(180,100%,50%,0.35)]" />
