...
serve(async (req) => {
  ...
  try {
    // --- Auth gate: a valid end-user JWT, OR a trusted internal call ---
    ...
    const isInternal = Boolean(internalSecret) && suppliedInternalSecret === internalSecret;
    ...
    // Resolve the target user: internal callers pass userId explicitly,
    // end-user callers are resolved from their JWT.
    let targetUserId: string;
    let targetUserEmail: string | undefined;

    if (isInternal) {
      if (!bodyUserId || typeof bodyUserId !== "string") {
        return json({ error: "Missing userId for internal call" }, 400);
      }
      const { data: adminUser, error: adminErr } = await adminClient.auth.admin.getUserById(bodyUserId);
      if (adminErr || !adminUser?.user) {
        return json({ error: "Unknown user" }, 400);
      }
      targetUserId = adminUser.user.id;
      targetUserEmail = adminUser.user.email ?? undefined;
    } else {
      ...
      targetUserEmail = userResult.user.email ?? undefined;
    }

    // --- Restrict recipient: only the target user's own email or saved notification_email ---
    ...
    const allowedRecipients = new Set(
      [targetUserEmail, pref?.notification_email].filter(Boolean).map((e) => String(e).toLowerCase()),
    );
...
<body style="margin:0;padding:0;background:#0a0a1a;font-family:Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:40px 20px;">
    ...
    if (response.ok) {
      ...
      return json({
        error: "Unable to deliver email at this time. Please try again later.",
        providerStatus: response.status,
      }, 502);
    }

    // Log the email alert to database (server-side, with service role)
    const { error: logError } = await adminClient.from("email_alert_logs").insert({
    ...
    if (logError) {
      console.error("Email delivered but alert log insert failed:", logError.message);
    }
