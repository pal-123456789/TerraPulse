...
serve(async (req) => {
  ...
  try {
    ...
    if (rateLimitError) {
      console.error("Detection rate-limit check failed:", rateLimitError.message);
...
Provide analysis in JSON format:
{
  ...
  "forecastTrend": "improving" | "stable" | "worsening" | "uncertain"
    ...
    if (analysis.hasAnomaly && analysis.confidence >= 60) {
      const { error: anomalyInsertError } = await serviceClient.from("anomalies").insert({
      ...
      if (anomalyInsertError) console.error("Anomaly insert failed:", anomalyInsertError.message);
      ...
      if (notificationError) {
        console.error("Anomaly notification insert failed:", notificationError.message);
      } else {
        console.log(`Anomaly in-app notification created for user ${userId}`);
      }
        ...
        if (prefs) {
          ...
          if (detectedIdx >= minIdx) {
            ...
            if (userEmail) {
              ...
              const emailRes = await fetch(`${supabaseUrl}/functions/v1/send-anomaly-email`, {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                  to: userEmail,
                  subject: `⚠️ TerraGuardians Alert: ${formatAnomalyName(analysis.anomalyType)}`,
                  anomalyName: formatAnomalyName(analysis.anomalyType),
