...
async function sendEmail(to: string, subject: string, html: string) {
  if (!RESEND_API_KEY || !LOVABLE_API_KEY) throw new Error("Resend email provider is not configured");
  const from = Deno.env.get("RESEND_FROM_ADDRESS") ?? "TerraGuardians Alerts <onboarding@resend.dev>";
...
Deno.serve(async (req) => {
  ...
  // spam email recipients or replay webhooks.
