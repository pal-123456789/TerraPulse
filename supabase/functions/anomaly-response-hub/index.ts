...
Deno.serve(async (req) => {
  ...
  const { data: claims, error: cErr } = await supabase.auth.getUser(authHeader.replace("Bearer ", ""));
  if (cErr || !claims?.user?.id) return json({ error: "Unauthorized" }, 401);
  const userId = claims.user!.id as string;
